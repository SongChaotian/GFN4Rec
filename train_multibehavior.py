...
if __name__ == '__main__':
    ...
    init_parser.add_argument('--reader', type=str, default='KRMBSeqReader', help='Data reader class')
    init_parser.add_argument('--model', type=str, default='KRMBUserResponse', help='User response model class.')
    ...
    # model and optimizer
    model = modelClass(args, stats, device)
    task_weight = torch.ones(model.feedback_dim)
    model.set_behavior_hyper_weight(task_weight)
    ...
    try:
        ...
        while epo < args.epoch:
            ...
            if improve > 0.6 * len(model.feedback_types):
