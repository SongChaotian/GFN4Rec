...
class MLSeqReader(BaseReader):
    ...
    def _read_data(self, args):
        ...
        # 格式: (user_id, movie_id, timestamp, is_click, is_like, is_star)
        
        # 2. 构建词表
        self.users = list(self.log_data['user_id'].unique())
        self.user_id_vocab = {uid: i+1 for i,uid in enumerate(self.users)}  # 从1开始编码

        self.items = list(self.log_data['movie_id'].unique())
        self.item_id_vocab = {iid: i+1 for i,iid in enumerate(self.items)}  # 从1开始编码
        ...
        self.padding_item_meta = {
            f: np.zeros_like(list(v_dict.values())[0]) for f, v_dict in self.item_vocab.items()
        }
...
#         position_id = row['position']
        ...
        record['history_length'] = hist_length  # 实际长度
